...
4. **On-demand "Check for source changes" action.** Given a thread that has prior snapshots for its bound sources, when the user triggers a "Check for source changes" action (a WS action Bob handles, mirroring how `_handle_clarify_answer` handles `clarify_answer` at [bob.py:1698-1727](src/ai_qa/agents/bob.py:1698)), then Bob re-fetches the bound sources, runs detection (AC1/AC2), emits the `source_change_report` (AC3), and does NOT regenerate requirements. This is a lightweight read-only check — it fetches sources and compares, then stops.
...
- [ ] **Task 3 — "Check for source changes" action (AC: 4)**
  - [ ] Add a Bob action handler (e.g. `_handle_check_source_changes`) dispatched from the WS action router the same way `clarify_answer` is. Trace the dispatch path in [api/websocket.py:315-388](src/ai_qa/api/websocket.py:315) (`_handle_action`) and how Bob exposes custom actions today (the `clarify_answer` precedent, [bob.py:1698-1727](src/ai_qa/agents/bob.py:1698)).
  - [ ] The handler re-fetches the thread's bound sources (reuse the existing reader path), runs detection, emits the report, and returns WITHOUT regenerating (read-only). It must respect the per-agent action lock so it doesn't interleave with an in-flight extraction ([api/websocket.py:360](src/ai_qa/api/websocket.py:360) lock, [[ws-nonblocking-clarify-timeout-fix]]).
...
- **Order matters (AC5).** Detect (read prior) strictly before 18.1's snapshot write, else the run diffs against itself. If 18.1 and 18.2 land together, enforce ordering in one place.
