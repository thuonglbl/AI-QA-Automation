diff --git a/_bmad-output/implementation-artifacts/6-3-role-based-access-control-for-admin-and-standard-users.md b/_bmad-output/implementation-artifacts/6-3-role-based-access-control-for-admin-and-standard-users.md
new file mode 100644
index 0000000..ffbd8c6
--- /dev/null
+++ b/_bmad-output/implementation-artifacts/6-3-role-based-access-control-for-admin-and-standard-users.md
@@ -0,0 +1,344 @@
+# 6-3: Role-Based Access Control for Admin and Standard Users
+
+## Header
+
+```yaml
+story_id: 6.3
+story_key: 6-3-role-based-access-control-for-admin-and-standard-users
...
+And current local-auth tests from Story 6.2 continue to pass
+```
+
+## Developer Context
+
+### Epic 6 Context and Boundaries
+
+Epic 6 is the course-correction epic that moves the application from a single-user, file-oriented workspace to a multi-user, project-scoped system. Story 6.1 established PostgreSQL/SQLAlchemy/Alembic persistence. Story 6.2 replaced JSON-file auth with PostgreSQL-backed local accounts, Argon2 password hashing via `pwdlib`, JWT cookie/bearer sessions, `/auth/me`, and admin bootstrap.
+
+This story must add backend authorization guardrails on top of that completed auth foundation. Treat Story 6.2 as the source of authenticated identity, but do not blindly trust role claims in old tokens when an endpoint needs authorization; re-check active user state from the database or through a shared current-user dependency.
+
+**Do implement:**
...
+
+**Do not implement:**
+- Frontend admin/user/project management screens; Story 6.6 owns frontend UI.
+- Full project list/member-scoped browsing behavior; Story 6.4 owns user-facing project and membership management depth.
+- Project-scoped artifact persistence; Story 6.5 owns artifact service.
+- Agent pipeline refactor from workspace paths to project context; Story 6.7 owns that.
+- Azure Entra ID/MSAL/SSO; Epic 22 remains deferred.
+
