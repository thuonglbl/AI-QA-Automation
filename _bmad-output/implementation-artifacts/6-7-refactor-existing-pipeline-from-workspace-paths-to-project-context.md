# Story 6.7: Refactor Existing Pipeline from Workspace Paths to Project Context
...
- `ArtifactService` already exists in [service.py](file:///src/ai_qa/artifacts/service.py). Reuse it; do not build a parallel persistence layer.
...
- Database tables already exist in [models.py](file:///src/ai_qa/db/models.py): `Project`, `ProjectMembership`, `PipelineRun`, `Artifact`, `ArtifactVersion`, `AuditEvent`, and `User`.
...
- Current pipeline dispatch lives in [routes.py](file:///src/ai_qa/api/routes.py) and [websocket.py](file:///src/ai_qa/api/websocket.py). Both currently route by user email and workspace-derived agents; this is the main seam to refactor.
...
- Story 12.7 source: [_bmad-output/planning-artifacts/epics.md](file:///_bmad-output/planning-artifacts/epics.md#L763-L778)
- Artifact service foundation: [service.py](file:///src/ai_qa/artifacts/service.py)
- Artifact storage foundation: [storage.py](file:///src/ai_qa/artifacts/storage.py)
- Artifact API authorization pattern: [artifacts.py](file:///src/ai_qa/api/artifacts.py)
- Pipeline dispatch: [routes.py](file:///src/ai_qa/api/routes.py)
- WebSocket dispatch: [websocket.py](file:///src/ai_qa/api/websocket.py)
- Database schema: [db/models.py](file:///src/ai_qa/db/models.py)
- Previous story 12.5: [_bmad-output/implementation-artifacts/12-5-project-scoped-artifact-service.md](file:///_bmad-output/implementation-artifacts/12-5-project-scoped-artifact-service.md)
- Previous story 12.6: [_bmad-output/implementation-artifacts/12-6-frontend-login-project-selection-and-api-client-foundation.md](file:///_bmad-output/implementation-artifacts/12-6-frontend-login-project-selection-and-api-client-foundation.md)
