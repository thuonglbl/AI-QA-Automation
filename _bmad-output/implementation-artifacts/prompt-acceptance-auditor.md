...
- âœ… Implemented `get_user_projects` in `src/ai_qa/projects/service.py` with SQLAlchemy tests.
- âœ… Refactored `GET /api/projects` in `src/ai_qa/api/projects.py` to use `get_user_projects`.
- âœ… Renamed `listProjects` to `getUserProjects` in `frontend/src/lib/projects.ts` to strictly match the requested API function name.
- âœ… Verified workspace shell handles zero-projects state gracefully in `App.tsx` (using the Professional Calm system).
