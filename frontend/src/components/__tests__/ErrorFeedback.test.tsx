...
describe('ErrorFeedback', () => {
  ...
  it('renders retry button with correct label', () => {
    ...
    const retryButton = screen.getByRole('button', { name: /retry this action/i });
  ...
  it('has screen reader text with error type', () => {
    ...
    expect(screen.getByText(`Error type: ${mockError.type}`)).toBeInTheDocument();
    expect(screen.getByText(`Error type: ${mockError.type}`)).toHaveClass('sr-only');
  });

  it('applies custom className', () => {
    const { container } = render(
      <ErrorFeedback error={mockError} onRetry={() => {}} className="custom-class" />
    );
