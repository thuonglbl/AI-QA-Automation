...
const defaultProps: ChatInputAreaProps = {
  ...
  inputConfig: {
    fields: [
      { name: 'url', label: 'Confluence URL', type: 'url' as const, required: true, placeholder: 'Enter URL' },
    ],
  },
...
describe('ChatInputArea', () => {
  ...
  describe('Review State (AC 3, 6, 8)', () => {
    ...
    it('calls onApprove when Approve is clicked', () => {
      ...
      render(<ChatInputArea {...defaultProps} state="review" onApprove={onApprove} />);
    ...
    it('calls onReject when Reject is clicked', () => {
      ...
      render(<ChatInputArea {...defaultProps} state="review" onReject={onReject} />);
  ...
  describe('Reject Feedback State (AC 4)', () => {
    it('renders textarea and Submit button', () => {
      ...
      expect(screen.getByPlaceholderText(/Describe what needs to be changed/i)).toBeInTheDocument();
    ...
    it('enables Submit button when feedback is entered', () => {
      ...
      fireEvent.change(textarea, { target: { value: 'Please fix this issue' } });
    ...
    it('calls onSubmitFeedback with feedback text when Submit is clicked', () => {
      ...
      render(<ChatInputArea {...defaultProps} state="reject_feedback" onSubmitFeedback={onSubmitFeedback} />);
  ...
  describe('Done State (AC 5)', () => {
    ...
    it('shows Completed button for final step', () => {
      render(<ChatInputArea {...defaultProps} state="done" stepNumber={5} isLastStep={true} />);
    ...
    it('calls onContinue when Continue is clicked', () => {
      ...
      render(<ChatInputArea {...defaultProps} state="done" onContinue={onContinue} />);
  ...
  describe('Focus Management (AC 9)', () => {
    it('focuses primary action on state change to start', async () => {
      render(<ChatInputArea {...defaultProps} state="start" inputConfig={{ fields: [{ name: 'test', label: 'Test', type: 'text', required: false }] }} />);
  ...
  describe('Accessibility (AC 7, 15)', () => {
    it('has aria-live for state announcements', () => {
      const { container } = render(<ChatInputArea {...defaultProps} state="start" />);
      ...
      const inputConfig = {
        fields: [
          { 
            ...
            type: 'text' as const, 
            required: false,
            validation: (value: string) => value.length < 3 ? 'Must be at least 3 characters' : null
          },
        ],
      };
